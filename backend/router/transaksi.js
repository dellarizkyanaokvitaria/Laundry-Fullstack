...
app.post("/",(request,response)=> {
    ...
    .then(result=> {
        ...
        let newIDTransaksi = result.detail_transaksi
...
app.put("/:id_transaksi",async(request,response)=> {
    //tampung data untuk update ke tbl transaksi
    let dataTransaksi = {
        id_member:request.body.id_member,
        tgl:request.body.tgl,
        batas_waktu:request.body.batas_waktu,
    ...
    .then(async(result)=> {
        ...
        //masukkan data detail yg baru
        let detail = request.body.detail_transaksi
        for(let i=0;i<detail.length;i++) {
            //sebelumnya
            //nilai detail[i] hanya puya key id_paket
            //dan qty saja
